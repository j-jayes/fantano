...
```{r}
video_album_query <- "Jackboys & Travis Scott - Self-Titled"
...
get_artist_data <- function(spotify_artist_id){
  ...
  spotify_artist_genres <- json_result_flat$genres %>% toString()
...
# test <- get_artist_data(test$spotify_artist_id)
...
get_track_features <- function(track_id){
  ...
  track_features <- json_result_flat %>% as_tibble() %>% select(danceability:tempo, duration_ms:time_signature)
...
# get_track_features("430qNtapCS3Ue1yoSql1oV")
```
