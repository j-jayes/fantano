...
tags %>% select(tags) %>% pull()
  

  mutate(tags = as.character(tags)) %>% 
  unnest_tokens(output = word, input = tags,format = "text", token = "regex", pattern = ";")

tags %>% tail() %>% select(tags)
...
basic_rec <- recipe(score ~ ., data = train) %>% 
    ...
    step_log(starts_with("statistic"), duration, base = 10) %>% 
